...
if "%VCVARS%"=="" if exist "%ProgramFiles%\Microsoft Visual Studio\2026\Community\VC\Auxiliary\Build\vcvars64.bat" (
...
if "%VCVARS%"=="" if exist "%ProgramFiles(x86)%\Microsoft Visual Studio\2022\BuildTools\VC\Auxiliary\Build\vcvars64.bat" (
...
if "%VCVARS%"=="" if exist "%ProgramFiles(x86)%\Microsoft Visual Studio\2019\BuildTools\VC\Auxiliary\Build\vcvars64.bat" (
...
if "%VCVARS%"=="" (
    ...
    echo.
    echo   Install steps:
    echo     1. Download Visual Studio 2022/2026 Community or Build Tools
...
echo   [1/4] Initializing MSVC toolchain...
call "%VCVARS%" >nul 2>&1
if errorlevel 1 (
    echo   [ERROR] vcvars64.bat failed!
    echo   Try opening "Developer Command Prompt for VS" from Start Menu
...
rc /nologo /fo dx11_sync.res resource.rc

if errorlevel 1 (
    echo.
    echo   ============================================================
    echo   [RESOURCE BUILD FAILED] See error details above.
    echo   ============================================================
    pause
    exit /b 1
)
...
echo.
echo   [3/4] Compiling (x64 Release, static CRT, zero deps)...

set "SRC=src\main.cpp src\app_window.cpp src\sync_engine.cpp src\window_manager.cpp src\key_blacklist.cpp"
set "OUT=dx11_sync.exe"
set "LIBS=user32.lib gdi32.lib advapi32.lib dwmapi.lib d2d1.lib dwrite.lib comctl32.lib"
set "OPTS=/nologo /EHsc /O2 /MT /W3 /utf-8"
set "DEFS=/D_WIN32_WINNT=0x0A00 /DNTDDI_VERSION=0x0A00000B /DWIN32_LEAN_AND_MEAN"

cl %OPTS% %DEFS% /Fe:%OUT% %SRC% /link dx11_sync.res %LIBS% /MANIFEST:NO /SUBSYSTEM:WINDOWS
